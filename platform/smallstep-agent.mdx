...
This guide covers manual installation of the Smallstep Agent on Linux, macOS, and Windows.
Use this guide if you
want to install the agent
via a software management tool separate from your MDM (eg Ansible, Munki),
or if your MDM only supports script-based software management.

For MDM-based deployments, see:
...
## Network Requirements (All Platforms)
...
2. Install the package on your endpoint (double-click the `.pkg` file, or use the `installer` command)
...
4. On the endpoint, register the launch agent by running:
...
1. Download the agent installer from the [Smallstep releases page](https://github.com/smallstep/step-agent-plugin/releases):
   ...
   - EXE installer also available: `step-agent-plugin-Setup_amd64_<version>.exe`
...
After installation and configuration, the agent should automatically register with your Smallstep team. You can verify registration in the Smallstep UI by checking the device's profile page for an **Enrolled At** timestamp in the **Device Registration** section.
