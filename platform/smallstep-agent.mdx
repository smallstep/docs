...
3. [Add your devices via API](./enrollment-guide.mdx#add-devices-via-api) so that they are pre-approved,
   ...
   Then enable the agent:
    ...
    The module renders `settings` to `agent.yaml` in the Nix store and links it at `/etc/step-agent/agent.yaml`,
    which is where the daemon and the `step-agent` CLI look for it.
...
services.step-agent.package = pkgs.step-agent.overrideAttrs (final: prev: rec {
  ...
  src = pkgs.fetchurl {
    ...
    hash = "sha256-BVJdru3gVVeiGGOPA+Vn9HZFQwusMPfjp3izSLmoOiY=";
  };
});
```
