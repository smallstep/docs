...
- Wi-Fi, using WPA3 Enterprise EAP-TLS. In this scenario, an Access Point delegates authentication to a RADIUS server. But the delegation flow is an inverted form of what delegated SSO looks like: The client authenticates directly with the authentication server, which then makes a decision and tells the access point (resource) what to do. Because of this inversion, there is no bearer token needed on the client side. Wi-Fi could be considered is a variant of both direct and delegated authentication methods.
