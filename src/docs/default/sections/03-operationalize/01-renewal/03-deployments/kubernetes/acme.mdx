...
<DocConfig>
  {({ kubernetes }) => (
    <p>
      ...
      <inlineCode>{kubernetes.ingressClass}</inlineCode>, and cert-manager will
      automatically intercept ACME requests to that ingress to solve the HTTP01
