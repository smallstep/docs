<p>
  ...
  already have a certificate, private key, and CA root certificate from your
  organization's existing CA, you can skip to the{' '}
  <DocConfig>{({ name }) => name}</DocConfig> TLS configuration section below.
  If you need to generate a certificate, you can:
</p>
<ul>
  <li>
    Use a{' '}
