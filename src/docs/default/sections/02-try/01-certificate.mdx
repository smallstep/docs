<p>
  Before you can configure <DocConfig>{({ name }) => name}</DocConfig> TLS you
  will need a certificate issued by a trusted certificate authority (CA). If you
  already have a <DocConfig>{({ name }) => name}</DocConfig> TLS certificate,
  private key, and CA root certificate from your organization's existing CA you
  can skip to the <DocConfig>{({ name }) => name}</DocConfig> TLS configuration
  section below. If you need to generate a certificate you can:
</p>
<ul>
  <li>
    Use a{' '}
