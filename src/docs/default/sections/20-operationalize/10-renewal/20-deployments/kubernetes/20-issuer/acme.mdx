...
spec:
  acme:
    ...
    - http01:
        ingress:
          class: ${props.doc.kubernetes.ingressClass || 'nginx'}`}
</CodeBlock>
...
<p>
  ...
  <inlineCode>{props.doc.kubernetes.ingressClass || 'nginx'}</inlineCode>, and
  ...
  "authenticates" your ACME client with the CA, and the CA will then sign the
  certificate.
</p>
...
<p>
  ...
  <inlineCode>{props.doc.server.name}-tls</inlineCode> secret with your
  certificate and private key. We'll reference this secret later when we
  configure {props.doc.name} to use this certificate and private key for TLS.
</p>
...
spec:
  acme:
    server: https://my-ca-url.my-team.ca.smallstep.com/acme/acme/directory
    privateKeySecretRef:
      name: ${props.doc.server.name}-tls
    solvers:
    - dns01:
        cloudDNS:
          project: my-project
