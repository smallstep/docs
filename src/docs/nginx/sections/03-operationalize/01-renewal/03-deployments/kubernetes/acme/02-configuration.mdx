...
in the `tls-myserver` secret, you'll need to configure Nginx
...
However, in Kuberentes, it's much more common to have Nginx
...
to make use of the certificate and private key in the `tls-myserver` secret
...
and automatically server the renewed certificate each time
