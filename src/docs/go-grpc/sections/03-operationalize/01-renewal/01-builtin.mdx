...
but if our certificate lifetime is 24 hours the renewal will kick on every
connection, to avoid this we can for example renew 8 hours before the expiration
setting `RenewBefore` to `8 * time.Hour`.

Here is the main function using `acme/autocert`:
...
does not support ii, it will automatically create a CSR with an RSA key size of
2048 bits and do the ACME flow to get it sign.

TLS cipher suites and the TLS version can be configured in the manager's
`*tls.Config`. The default behavior is to use a safe list of all the ones
