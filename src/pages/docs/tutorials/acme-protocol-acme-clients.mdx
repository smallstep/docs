...
### ACME and open source `step-ca` Recap
...
Replace `<step path>` with the output of the [`step path`](/docs/step-cli/reference/path/) command.
...
You can use the `--exec` flag on the [`step ca renew`](/docs/step-cli/reference/ca/renew) command to do this automatically:
