...
- **[Smallstep Certificate Manager](https://smallstep.com/certificate-manager) -** Please visit the [OIDC documentation](/docs/certificate-manager/oidc) instead of using this tutorial to connect keycloak to Certificate Manager.
...
If `step-ca` raises an OIDC token validation error, you can examine the token you receive from Keycloak using the [`step oauth`](/docs/step-cli/reference/oauth/) command. For example:
...
* The email address used with [`step ssh login`](/docs/step-cli/reference/ssh/login) doesn't match the email address of the Keycloak user.
