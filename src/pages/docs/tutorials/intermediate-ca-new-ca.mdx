...
- **[Smallstep Certificate Manager](/certificate-manager) -** Please contact [Smallstep Customer Success](mailto:support@smallstep.com) and we will assist in creating your intermediate authority off an existing Root.
...
It's easiest to run [`step ca init`](/docs/step-cli/reference/ca/init) to get the boilerplate configuration in place, then remove or replace these artifacts with new ones that are tied to your existing root CA.

```shell-session 
$ step ca init
```

When you run [`step ca init`](/docs/step-cli/reference/ca/init) we create a couple artifacts under `~/.step/`. The important ones for us are:
...
Now you need to generate a new signing key and intermediate certificate signed by your existing root CA. To do that, we can use the [`step certificate create`](/docs/step-cli/reference/certificate/create) subcommand to generate a certificate signing request (CSR) that we'll have your existing root CA sign, producing an intermediate certificate.
