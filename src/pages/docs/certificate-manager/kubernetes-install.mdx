...
When following Kelsey Hightower’s popular Kubernetes the Hard Way instructions you will be guided to use `cfssl` for generating 1yr certs for the following components: etcd, kube-apiserver, kube-controller-manager, kube-scheduler, kubelet, and kube-proxy. 
...
We think issuing a 1yr certificate and moving on is setting you up for pain 365 days later. 
So much so that we [blogged about a better way](/blog/kubernetes-the-secure-way) and created a pull request to the Hightower repo. 
You should certainly check it out if you are building Kubernetes the hard way.
