...
With ACME, certificate issuane, renewal and revocation can be automated.
...
As the name implies, EAB allows (new) ACME accounts to be bound (or linked) to an account in another (external) system.
This allows configuration of which accounts, including users and automated systems, can interact with the ACME server.

TODO: include some text on threat model, what attacks are (still) possible?