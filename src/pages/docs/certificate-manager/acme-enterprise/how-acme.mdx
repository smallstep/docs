...
When deploying ACME, it's possible to configure the platform with some additional controls for certificate issuance.
...
Certificate issuance policies apply to all authorities and provisioners, and is thus not specific to the ACME functionalities.
Combining ACME and an authority or provisioner policy can result in a simpler deployment, however, if the policy only allows hostnames for specific domains to be signed.
Unknown third party ACME clients try to request a certificate, but if they're not requesting certificates for the allowed (sub)domains, no certificate will be issued.

### External Account Binding

ACME EAB can be used to bind an ACME account to another (external) system.
Only ACME clients that were provided with a shared secret will be able to register an account with the CA.
This allows an administrator to link the (future) ACME account to an existing entity in a different system, such as an inventory of machines.

When EAB is in use, it's possible to configure a certificate issuance policy on the ACME account level, resulting in more granular control over certificate issuance.
