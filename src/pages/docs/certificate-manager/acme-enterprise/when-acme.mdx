...
- An environment requiring certificates for Mutual TLS, such as a service mesh.
    ...
    Considering the clients in those environments are often primarily concerned with infrastructural concerns, they can be issued a certificate using ACME if then can solve challenges.
