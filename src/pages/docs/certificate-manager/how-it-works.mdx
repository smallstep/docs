...
Certificate Manager endpoints receive and renew certificates from Authorities.
Endpoints are automatically identified by certificate characteristics across issuances.
* Common SANs
* The ACME account for ACME workflows
* The email or SAN for OIDC workflows

The Certificate Manager dashboard provides an endpoint detail view (coming soon). 
This makes it easy for an administrator to view the historical certificate linage for a single endpoint.
This aggregation also simplifies auditing, reporting, and compliance inquiries. 

Smallstep also bills by the number of endpoints based on the metered time that begins at
* The endpoint first certificate's `not before` date, until
* The endpoint most recent certificate's `not after` date.

Let's look at two endpoint examples:
1. Long-lived or one-time certs
   
   If you issue a certificate and never renew or replace it, you will be metered hourly and billed monthly for the life of the certificate.

   In this example, a single device with **one 365-day certificate** would be billed as an endpoint for 12-months. 

2. Automated short-lived certificates
   
   If you issue a certificate, then (a) renew it, (b) reissue with same ACME account or (c) reissue with same email/SAN using OIDC, you will be metered hourly and billed monthly from the earliest `not before` to the latest `not after` date.
   
   In this example, a single device with **365 one-day certificates renewed every 12-hours** would be billed as an endpoint for 12-months. 

Endpoints not only simplify reporting and management but also promote industry best practices around short-lived certificates. 
