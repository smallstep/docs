...
# Webhook Events
The Webhook Events allow for real-time logging of SSH sessions, 
...
- **Expiration**: Sends a webhook event for Certificate Manager when a certificate is within the last 25% of its lifetime and when it expires.
- **Certificate**: Sends a webhook event for Certificate Manager with basic cert details whenever a certificate is created or expires.
- **All**: Send events for everything above for both SSH and Certificate Manager
...
To register a webhook event:

1. Go to Settings > webhooks
2. Provide necessary information as shown below:

    ![image](/graphics/webhook-events-demo.png 'Webhook Events Requirements')

## Webhook Events Response Formats
For the different webhook events, here are examples and details of the response data you should expect to receive:  
