...
Clients using the `step` CLI can then authenticate with the Nebula provisioner using their Nebula host certificate and private key.
Clients are then allowed to request an X.509 or SSH certificate with any of the hostname(s) that appear on their Nebula host certificate.

Create a Nebula provisioner:

```shell
step beta ca provisioner add --type Nebula --nebula-root /etc/nebula/ca.crt
```

Note that the Nebula provisioner is only supported by the `step beta ca provisioner` command (and a database-backed provisioner configuration). It is not yet supported to configure a Nebula provisioner using `ca.json`.
