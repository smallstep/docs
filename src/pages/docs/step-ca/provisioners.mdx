...
To use the default flow for input-constrained devices, run:

```shell-session
$ step ca certificate foo foo.crt foo.key --console
```

To specify a flow other than the default (for example Google's deprecated Out of Band flow) as follows:
