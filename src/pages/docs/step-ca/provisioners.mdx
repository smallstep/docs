...
"Super Admins" are Admins that can also manage the list of Admins for the CA.
...
<Alert severity="info">
  <div>
	...
	<p>Only Admins (or Super Admins) can manage CA provisioners remotely.
	...
	The default Admin user is a Super Admin called <Code>step</Code>.
...
For the default JWK provisioner, the `step` command will prompt for the provisioner password. Behind the scenes, it creates and signs a CA authentication token (a JWT) with the administrator's username in it.
