...
"Super Admins" are Admins that can also manage the list of Admins for the CA using [`step ca admin`](/docs/step-cli/reference/ca/admin).
...
ACME clients must answer challenges presented by the ACME server to prove to the that they control the identifiers listed in the CSR.
...
See [ACME Basics](/docs/step-ca/acme-basics#acme-challenges) for a description of each challenge type and their tradeoffs.
...
The CSR may include a device ID in a `permanentIdentifier` SAN, depending on the application.

**This challenge type is not enabled by default.**
Enable it in a new ACME provisioner by running:

```bash
step ca provisioner add <name> --type ACME --challenge device-attest-01
```

Apple's device attestation CA is trusted by the ACME provisioner by default.
...
* An [`ACMECertificate`](https://developer.apple.com/documentation/devicemanagement/acmecertificate) payload. For this one, set the `ClientIdentifier` to the UDID of the device. 
