...
#### Need A Custom SCEP Path?

For compatibility with SCEP clients that expect a specific path segment in the SCEP URL, an optional `customPath` parameter can be supplied in the SCEP provisioner configuration. If the above example configuration were extended with `"customPath": "/pkiclient.exe"`, the SCEP provisioner would be available at `http://ca.example.com:8080/scep/scepca/pkiclient.exe`.
