...
Counterintuitively, this is a secure implementation of OAuth that conforms to the OAuth Best Current Practices for for Native Apps (<a href="https://www.rfc-editor.org/rfc/rfc8252.html">RFC8252 / IETF BCP212</a>).
...
This constraint obviates the need for a client secret, because a `localhost` flow is inherently resistant to network attacks that the client secret is designed to mitigate in other, non-native flows.
...
So, the client secret serves no purpose in the Authorization Code flow for native apps.
...
Bottom line, this OAuth flow is widely vetted and considered secure.
  </div>
</Alert>
