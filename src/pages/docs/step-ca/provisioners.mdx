...
See [`step ca provisioner add --help`](/docs/step-cli/reference/ca/provisioner/add) for documentation and examples on adding provisioners.

We strongly recommend using the [`step ca provisioner add [...]`](/docs/step-cli/reference/ca/provisioner/add) utility
to generate provisioners in your `ca.json` configuration.
We often encode fields differently in the JSON than you might expect.
And you can always modify the JSON configuration manually after using the utility.

#### Remove a provisioner

See [`step ca provisioner remove --help`](/docs/step-cli/reference/ca/provisioner/remove) for documentation and examples on removing provisioners.
...
There are optional configuration modifiers that apply to all provisioner types.
...
- **disableRenewal**: do not allow certificates to be renewed. The default is false.

- **allowRenewalAfterExpiry**: ☠️  allow expired certificates to be renewed. The default is false. This option adds security risk; proceed with caution and consider alternative approaches first.
...
Learn more about [configuring provisioner templates](/docs/step-ca/templates).
...
provisioner when [`step ca init`](/docs/step-cli/reference/ca/init) is used, and it also contains commands to add
([`step ca provisioner add`](/docs/step-cli/reference/ca/provisioner/add)) and remove ([`step ca provisioner remove`](/docs/step-cli/reference/ca/provisioner/remove)) JWK
provisioners.
...
1. Retrieve the current encrypted key.
  ...
  ```shell
  OLD_ENCRYPTED_KEY=$(jq -r \
     '.authority.provisioners[] | select(.name == "carl@example.com").encryptedKey' \
