...
- **authMountPath**: the vault mount path for the auth method you want to use, if not set used the default mount path for that auth type (usually the same name as the auth method).
