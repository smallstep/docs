...
In this tutorial, we'll create a PKI and a Certificate Authority on our Vault server
Then we'll configure a `step-ca`, in RA mode, to use our Vault server to sign certificates.
...
You'll also need policies for each role.
Run:
...
If you know your PKI will only use a subset of tehse key types, you could limit this policy.
...
Note that the Vault `secretID` can be read from several sources.
You can hardcode it into the configuration file using `fromString`, as shown here.
Or you can use `fromFile` and provide a filename, or `frontEnv` and provide an environment variable name.
We recommend using `fromFile` in production environments.
