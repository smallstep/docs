...
and the webhook data is passed along to the provisioner's [certificate template](/docs/step-ca/templates).

Attach webhooks to [provisioners](/docs/step-ca/provisioners) via the `step ca provisioner webhook` command group.
...
<Alert severity="warning">
  ...
  <div>
    Webhooks enrich certificates.
    They should not be used to delegate CSR authorization to an external server.
    Webhooks are run early in the CA request processing cycle,
    and if policy decisions are made at this point
    it can create confusing interactions with other policy features of the CA.
  </div>
</Alert>
...
For this, there's three authentication schemes available.
We recommend adopting at least two of them:
- **Signature**  
  The CA sends a signature of its payload in the webhook request header. The webhook server must confirm this signature.
...
configure your webhook server should to request and verify a client certificate
that chains up to your root CA certificate.

This behavior also can be disabled:
