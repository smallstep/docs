...
An example DSN that enables TLS server hostname verification is shown below:
...
The behavior of the database driver mimics that of `libpq` and also encompasses handling of other default settings and environment variables.
More info about the options that `libpq` provides can be found at https://www.postgresql.org/docs/current/libpq-ssl.html.
