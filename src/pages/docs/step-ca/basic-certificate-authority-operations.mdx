...
Note that default maximum certificate duration is 24 hours. To adjust it, add a `claims` section to the `ca.json` configuration file, either under `"authority"` or under a specific provisioner's configuration block, with the following keys:
