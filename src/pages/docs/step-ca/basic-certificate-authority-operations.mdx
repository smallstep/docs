...
- [Working With X.509 Certificates](#working-with-x-509-certificates)
...
Once you have a certificate authority up and running, the [`step ca certificate`](/docs/step-cli/reference/ca/certificate) command is a one-step option for generating a private key and obtaining a signed certificate:
...
The positional arguments in the [`step ca certificate`](/docs/step-cli/reference/ca/certificate) command indicate the name you would like associated with the certificate (in this case, the DNS host name of a service) and the output filenames for the certificate and private key.

You can check your work using [`step certificate inspect`](/docs/step-cli/reference/certificate/inspect):
...
Let's make a CSR, using [`step certificate create --csr`](/docs/step-cli/reference/certificate/create), then we'll use [`step ca sign`](/docs/step-cli/reference/ca/sign) to get a signed certificate from the CA:
...
Under the hood, many of `step-ca`'s [certificate provisioners](/docs/step-ca/certificate-authority-core-concepts/#provisioners) accept a Javascript Web Token (JWT) along with a CSR to authenticate certificate requests:
...
When it comes time to renew your certificate, do not dawdle: Once a certificate expires, the CA will not renew it. To avoid catastrophe, you should [set up automated renewals](/docs/step-ca/renewal) for any certificate that always needs to be valid. Our automation methods will attempt renewal at around two-thirds of a certificate's lifetime.

Note that because [`step ca renew`](/docs/step-cli/reference/ca/renew) uses mutual TLS authentication, it can only renew client certificates (certificates that are marked with "Client Authentication" key usage). Any other certificate must be replaced before expiry instead.
...
See [Basic Configuration Options](/docs/step-ca/configuration#basic-configuration-options) for more details.
...
For more on this topic, read [All About Certificate Revocation](/docs/step-ca/revocation).
...
When [`step ca init`](/docs/step-cli/reference/ca/init) is run with `--ssh`, it creates two SSH CA key pairs: one for the host CA, and one for the user CA. The user CA key signs SSH user certificates, and the host CA key signs SSH host certificates.

### Requirements

* You will need a `step-ca` Certificate Authority **with SSH support enabled**. Create one by running [`step ca init --ssh`](/docs/step-cli/reference/ca/init).
...
* Learn to automate certificate management in your PKI. [ACME Basics](/docs/step-ca/acme-basics) introduces the ACME protocol for certificate automation, and includes a tutorial for setting it up with `step-ca`.
* See [Provisioners](/docs/step-ca/provisioners), [Production Considerations](/docs/step-ca/certificate-authority-server-production), and our [Configuration Guide](/docs/step-ca/configuration) to learn more about tailoring `step-ca` to your infrastructure.
* Run [`step ca help`](/docs/step-cli/reference/ca) or [`step ssh help`](/docs/step-cli/reference/ssh) for many more flags and examples.
* Our [Tutorials](/docs/tutorials) can guide you through more complex scenarios for using `step-ca` in various contexts and workflows.
