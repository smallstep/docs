...
<Alert severity="warning">
  <div>
    You must first enable <Link
      external
      href="https://smallstep.com/docs/step-ca/provisioners/#remote-provisioner-management"
    >
	Remote Provisioner Management
    </Link> in order to configure policies using the <Code>step</Code> CLI tool.
    Alternatively, you can hand-edit <Code>ca.json</Code> to add policies.
	See <Link
	  external
	  href="https://smallstep.com/docs/step-ca/policies#policy-in-configuration-file">
...
<Alert severity="info">
  ...
  <div>
    The authentication scheme for Remote Management uses short-lived X.509 certificates to authenticate (super) admin users.
    ...
    The policy can then be adjusted to include the (super) admin subject(s), e.g. <Code>step ca policy authority x509 allow dns step</Code>.
  </div>
</Alert>
