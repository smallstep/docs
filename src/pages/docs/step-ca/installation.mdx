...
1. Install `step`
    ...
    tar -xf step.tar.gz
    sudo cp step_0.20.0/bin/step /usr/bin
    ```
...
2. Install `step-ca`
    ...
    ```shell
    wget -O step-ca.tar.gz https://dl.step.sm/gh-release/certificates/docs-ca-install/v0.20.0/step-ca_linux_0.20.0_amd64.tar.gz
    tar -xf step-ca.tar.gz
    sudo cp step-ca_0.20.0/bin/step-ca /usr/bin
    ```
