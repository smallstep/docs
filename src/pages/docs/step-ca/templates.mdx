...
- **.Insecure.User**:
  This propert holds user-supplied data from the certificate request.
...
- **.Insecure.CR.Principals**: If you trust a host to register its own custom SANs
  ...
  then the host can pass several `--principal` values to [`step ssh certificate`](/docs/step-cli/reference/ssh/certificate) when registering the host,
...
- **.Insecure.User**: This property holds user-supplied data from the certificate request.
  Users can supply arbitrary values using `--set` or `--set-file` flags in [`step ssh certificate`](/docs/step-cli/reference/ssh/certificate).
...
The command [`step certificate create`](/docs/step-cli/reference/certificate/create) with the `--profile intermediate-ca` flag
...
The command [`step certificate create`](/docs/step-cli/reference/certificate/create) with the `--profile root-ca` flag uses the
...
[`step ca certificate`](/docs/step-cli/reference/ca/certificate) and [`step ca sign`](/docs/step-cli/reference/ca/sign). If you need to pass more than one
...
Finally, test out your flow with [`step ssh login`](/docs/step-cli/reference/ssh/login), and use [`step ssh inspect`](/docs/step-cli/reference/ssh/inspect) to check the principals of the certificate you're issued by the CA.
...
[`step certificate create --csr`](/docs/step-cli/reference/certificate/create).
