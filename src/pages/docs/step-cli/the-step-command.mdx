...
By default, the `step` toolchain assumes a single CA is in use, and it will store that CA's configuration and state directories (`config`, `certs`, `secrets`, `db`, etc.) inside a single folder, `$HOME/.step`.
...
Every context consists of a configuration profile directory (which includes user-managed [`defaults.json`](#configuration-file) and `ca.json`), and a directory for CA-managed data (CA roots, certificates, secrets, templates).

To enable contexts, pass the `--context` flag to any of the `step` setup commands (`step ca bootstrap`, `step ca init`, `step ssh config`), along with a label for your new context:

```bash
$ step ca bootstrap --ca-url https://ca:4443 --fingerprint c8d3...7aa --context vpn
The root certificate has been saved in /home/carl/.step/authorities/ca/certs/root_ca.crt.
The authority configuration has been saved in /home/carl/.step/authorities/ca/config/defaults.json.
The profile configuration has been saved in /home/carl/.step/profiles/ca/config/defaults.json.
$ step context current
vpn
```

When contexts are enabled:
- User configurations profiles are now stored in the `profiles` directory, and CA-managed data is stored in `authorities` directory.
- Context configuration files `contexts.json` and `current-context.json` are created.
...
There is no support for automatically migrating a single-profile `$STEPPATH` configuration into its own context. Enabling contexts will preserve any previously existing configuration or state data in `$STEPPATH`, but it will not migrate your previous configuration into its own new context.
