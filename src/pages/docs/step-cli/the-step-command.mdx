...
The `step` command supports plugins for adding command groups.
A plugin is an executable file called `step-COMMANDNAME` located in `$PATH` or `$STEPPATH/plugins`.
For example, a plugin named `$STEPPATH/plugins/step-dns` will execute when `step dns` is run.
