...
- There is always a currently active context (`step context current`), but you can pass a `--context` name to `step ca`, `step ssh`, or `step-ca` to temporarily select a context for a single operation.
