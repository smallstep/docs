...
The `step` command supports plugins. A plugin is an executable file called `step-COMMANDNAME` located in `$PATH` or `$STEPPATH/plugins`. For example, a plugin named `$STEPPATH/plugins/step-dns` will execute when `step dns` is run. If you're writing a plugin, please follow our convention for CLI help by implementing eg. `step dns help`.
