...
- There is always a currently active context ([`step context current`](/docs/step-cli/reference/context/current)), but you can pass a `--context` name to any [`step ca`](/docs/step-cli/reference/ca) or [`step ssh`](/docs/step-cli/reference/ssh/) command to temporarily select a context for a single operation.
