...
## Secure a `step-ca` host for ACME

The `dns-01`, `http-01`, and `tls-alpn-01` challenge types
depend on the CA having a trusted DNS service.
You can configure the DNS resolver on your `step-ca` host
to use either DNS over TLS or DNSSEC to authenticate its queries with DNS servers.
Traditional, unauthenticated DNS is not recommended.

## Configure a `step-ca` server for ACME
