...
An extension to ACME called External Account Binding (EAB)
adds an authorization layer,
...
No single standard way exists to prove control over an identifier, so the
...
Where the other challenge types prove control of a domain or IP address to secure a *server*, `device-attest-01` proves a *device's* hardware identity—the basis of ACME device attestation (ACME DA).
...
Further reading:
* [ACME Device Attestation](https://smallstep.com/platform/acme-device-attestation/)—Smallstep's platform for issuing hardware-bound device certificates with ACME DA, natively supported on macOS, iOS, Windows, Android, and Linux.
