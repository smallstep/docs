...
* [`step ca renew`](../step-cli/reference/ca/renew) typically uses mutual TLS authentication, but with a StepCAS RA it must use X5C authentication. Pass `--mtls=false` to [`step ca renew`](../step-cli/reference/ca/renew) when working with RAs. Vault RA mode does not support renewal.
...
Hashicorp Vault RA mode does not support certificate renewal.
