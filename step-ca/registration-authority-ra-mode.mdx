...
StepCAS supports the [JWK](../step-ca/provisioners.mdx#jwk) and [X5C](../step-ca/provisioners.mdx#x5c) provisioners for requests between RAs and the CA.
...
You will need the name and password of the first provisioner you created when running [`step ca init`](../step-cli/reference/ca/init.mdx).
...
* [`step ca renew`](../step-cli/reference/ca/renew.mdx) typically uses mutual TLS authentication, but with a StepCAS RA it must use X5C authentication. Pass `--mtls=false` to [`step ca renew`](../step-cli/reference/ca/renew.mdx) when working with RAs.
* [`step ca rekey`](../step-cli/reference/ca/rekey.mdx) is not supported.
...
4. Initialize your PKI.
  ...
  Run [`step ca init --ra cloudcas`](../step-cli/reference/ca/init.mdx) to create a new PKI in CAS.
