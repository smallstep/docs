...
4. Get a certificate for the YubiKey from `step-ca`.
   ...
   ```bash
   {
     ...
     step ca certificate \
     --attestation-uri 'yubikey:slot-id=9a?pin-value=123456' \
     --provisioner acme-da \
     $YUBIKEY_SERIAL_NO yubikey.crt
   }
   ```
...
* **options**<Reference id="star6" marker="*" />: with the Wire integration enabled, the certificate template can include (transformed) properties from the OIDC and DPoP tokens (for example, `.Oidc.preferred_username`, and `.Dpop.sub`). See [template configuration](templates.mdx#configuring-step-ca-to-use-templates) for more template configuration details.
...
K8sSA tokens are very minimal. No place exists for SANs, or other details that
...
- **Disable Custom SANs, if possible.**
  When using the `disableCustomSANs` setting, the CA will only issue certificates with authentic names extracted from the signed instance identity document.
...
Many details must be configured correctly to make this model secure, and many of these details are environment-dependent and beyond the scope of this document.
