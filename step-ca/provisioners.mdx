...
* The `tpm` format is a variant of the [WebAuthn TPM Attestation statement](https://www.w3.org/TR/webauthn-2/#sctn-tpm-attestation). By default, it does not add trust for any TPM vendor CAs. You must add TPM vendor trust roots via the `attestationRoots` configuration.
...
TPMs also have privacy constraints that complexify attestation flows.
