...
<Footnote id="f2" marker="2"> Privileged OIDC subject names can generate Host SSH Certificates. These can be configured in the <Link href="#oauthoidc-single-sign-on">OIDC provisioner</Link>.</Footnote>
...
Use the [`step ca provisioner`](../step-cli/reference/ca/provisioner.mdx) command group to add, remove, or modify provisioner configurations. Run these commands directly on your CA machine. They need to modify the `$(step path)/config/ca.json` configuration file.
...
[run several load-balanced](../step-ca/certificate-authority-server-production/#load-balancing-or-proxying-step-ca-traffic) `step-ca` instances,
...
Use [`step ca provisioner add`](../step-cli/reference/ca/provisioner/add.mdx) to add a provisioner:

```bash
step ca provisioner add acme --type ACME
```

See the [command reference](../step-cli/reference/ca/provisioner/add.mdx) for complete documentation and examples.

#### Remove a provisioner

Use [`step ca provisioner remove`](../step-cli/reference/ca/provisioner/remove.mdx) to remove provisioners:
...
To get a list of all of your current provisioners, use [`step ca provisioner list`](../step-cli/reference/ca/provisioner/list.mdx) or the `/provisioners` endpoint on your CA.

#### Modify a provisioner

Use [`step ca provisioner update`](../step-cli/reference/ca/provisioner/update.mdx) to update provisioner configurations:
...
Provisioner configuration is managed by running [`step ca provisioner`](../step-cli/reference/ca/provisioner.mdx) commands locally or remotely.
...
"Super Admins" are Admins that can also manage the list of Admins for the CA using [`step ca admin`](../step-cli/reference/ca/admin.mdx).
...
You can use the [`step ca provisioner`](../step-cli/reference/ca/provisioner.mdx) commands, from any client, to modify your CA's provisioner configuration.
...
The [`step`](https://github.com/smallstep/cli) CLI tool will create a JWK provisioner when [`step ca init`](../step-cli/reference/ca/init.mdx) is used.
...
- **options**<Reference id="star2" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
We can use [`step crypto jwe decrypt`](../step-cli/reference/crypto/jwe/decrypt.mdx) to see the private key encrypted with the password `asdf`:
...
- **options**<Reference id="star4" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
- **options**<Reference id="star5" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
- **options**<Reference id="star6" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
Because [`step ca init`](../step-cli/reference/ca/init.mdx) creates an ECDSA chain by default, you will need to [convert your CA to use an RSA CA chain](../tutorials/rsa-chain.mdx) before using the SCEP provisioner.
...
- **options**<Reference id="star7" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
A host can then renew its certificate using [`step ca renew`](../step-cli/reference/ca/renew.mdx).
...
- **options**<Reference id="star8" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
...
- **options**<Reference id="star10" marker="*" />: see [template configuration](templates#configuring-the-ca-to-use-templates) for more details.
