...
- **scopes**: a list of scopes OAuth clients should request. This is used by the `step` CLI. The default value is `["openid","email"]`. Note: `step oauth` also accepts a `--scope` flag for supplying scopes on the client side.
