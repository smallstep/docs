...
While we think `step-ca` is the best open source, scalable online Certificate Authority on the internet, every piece of software comes with limitations and tradeoffs.

`step-ca` is designed to favor a simple deployment of a scalable two-tiered X.509 PKI, with one Root CA and one Intermediate CA that issues end-entity certificates with passive revocation. Its SSH support is favors having two single-tier SSH CAs—a single SSH Host CA, and a single SSH User CA.
