...
- Issue certificates to humans, for example, using Single Sign-on (OpenID Connect) for authentication.
...
- [OAuth OIDC single sign-on tokens](https://smallstep.com/blog/easily-curl-services-secured-by-https-tls.html), for example:
...
- [Single-use, short-lived JWK tokens](./provisioners.mdx#jwk), for example, issued by your CD tool—Puppet, Chef, Ansible, Terraform, etc.
