...
- **kms**: enables and configures cryptographic protection using cloud key management services or hardware security modules. See [Cryptographic Protection](#cryptographic-protection).

  - **type**: required. one of `azurekms`, `awskms`, `cloudkms`, `pkcs11`, or `yubikey`

  - **uri**: this field can be used to specify other fields in this section, and its value takes precedence over those values. See [cryptographic protection](#cryptographic-protection) for examples.
...
implementations to store persistent certificate management metadata.
Our recommended default database implementation is nosql-Badger,
a NoSQL interface over the popular [Badger](https://github.com/dgraph-io/badger) database.
The database layer stores every certificate
(along with metadata surrounding the provisioning of the certificate) and
revocation data used to enforce passive revocation.
...
- `type`
  ...
  - **badger**: currently refers to Badger V1. Some day, when Badger V1 is deprecated, this will refer to Badger V2.
...
The [`libpq` docs](https://www.postgresql.org/docs/current/libpq-ssl.html) cover TLS options and the DSN query parameters for `libpq`.
