...
- [Certificate Lifecycle Management](#certificate-lifecycle-management)
  - [Automate Renewals](#automate-x509-certificate-lifecycle-management)
  - [Certificate Revocation](#x509-certificate-revocation)
  - [Rotate CA Certificates](#rotating-ca-certificates)
...
- Enumeration of CA provisioners. The `/provisioners` endpoint lists all provisioners and is included for client convenience. Without it, clients will need to know a provisioner name to use and, for JWK provisioners, the provisioner's private key.
- Exposed, unauthenticated ACME provisioners. By default, any ACME client that can reach the CA and satisfy an ACME challenge can enroll in your PKI.
- Enumeration of SSH infrastructure. The `/ssh/hosts` endpoint lists all enrolled hosts.

### Approach: Run A Reverse Proxy
...
Generally speaking,
it makes sense to create an allowlist of endpoints in a reverse proxy server
—the minimum to allow your clients to enroll—
and only forward those requests to step-ca.

#### Safely Proxying an ACME Provisioner

By default, the ACME provisioner issues certificates to any client that can satisfy an ACME client.
So, if you are connecting an ACME provisioner to the internet, take precautions:
...
- Hide your ACME provisioner:
  1. Use an unguessable shared secret as your ACME provisioner name:
  ...
  2. Block internet traffic to `/provisioners`, and the ACME provisioner URL cannot be discovered.

### Approach: Use Federation

If your clients are spread across differenet networks—
eg. in multi-cloud environments—
you can use [multiple autonomous federated CAs](../tutorials/pki-trust-model-federation.mdx)
instead of exposing a single CA.
