...
While `step-ca` does directly support Certificate Revocation Lists (CRLs) it does not directly support Online Certificate Status Protocol (OCSP). In order to enable CRL distribution use the following approach.
...
2. Add the following to `ca.json` file:
  ...
  "crl": {
    "enabled": true,
    "idpURL": "http://ca.internal/1.0/crl"
  },
  "..."
  ```

3. Enable `crlDistributionPoints` within your default template:
  ...
  {{- end }}
	...
	"crlDistributionPoints": ["http://ca.internal/1.0/crl"]}
  ```

4. The approach above assumes serving CRL over http. If you use Caddy to proxy the traffic add the following to `Caddyfile` to allow hosts to reach the CRL distribution point over http:

  ```
  # This serves crl over http
  http://ca.external {   
    # Backend connection to step-ca
    reverse_proxy http://ca.internal:9001
  }
  ```

5. Finally, configure your `step-ca` server to use the intermediate CA you created.

6. Once you revoke an x509 certificate, check the CRL served:

  ```bash
  step crl inspect --ca $(step path)/certs/root_ca.crt http://ca.internal:9001/1.0/crl
  ```
