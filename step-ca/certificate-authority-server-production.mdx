...
Root CA rotation is trickier. It requires a migration period. You'll need to create a new root certificate and add it as a trusted CA to all of your current endpoints before you can start signing new certificates with the new root. Furthermore, root CA rotation will always require intermediate rotation, since the intermediate CA is signed by the root CA.

### Rotating an Intermediate CA

Intermediate CA rotation will not affect previously-issued leaf certificates. These certificates are already bundled with the old intermediate CA. When an old certificate is renewed, the entire PEM bundle is replaced. This way, intermediate CA rotation can be seamless.
...
2. Sign a new intermediate CA using your root CA:
   ```bash
   $ step certificate create "Example Intermediate CA" \
        $(step path)/certs/intermediate_ca.crt \
	      $(step path)/secrets/intermediate_ca_key \
...
In rare cases, an intermediate CA must be bundled with a trusted root CA.
In this case, you will need to rotate those CA bundles when you rotate the intermediate CA.

If you have this sort of CA bundle in your infrastructure, you can smooth the transition to your new intermediate.
adding both the new and old intermediate CA certificates to the bundle
during a transition period.
Remove the old intermediate from the bundle once all of the certificates signed by it have expired. This is similar to the root rotation process.

### Rotating a Root CA

To rotate your root CA:
...
1. Create a new root CA.
   ...
   $ step certificate create "Example Root CA" \
        ...
        --profile root-ca
    ```
   Choose a strong password for your root CA. You'll need it to sign an intermediate CA.
2. Add `root_ca.crt` to the trust stores on all of your active endpoints. Do not remove your previous root CA from any trust stores yet.
3. Update any `step ca bootstrap` commands to use the new root CA fingerprint.
3. Back up your previous root CA certificate (`$(step path)/certs/root_ca.crt`) and private key (`$(step path)/secrets/root_ca_key`)
4. Stop `step-ca` and copy your new root CA into place.
5. Finally, follow the instructions for [rotating an intermediate CA](#rotating-an-intermediate-ca).
6. Copy `root_ca.crt` into `$(step path)/certs`.
6. Restart `step-ca` to pick up the new root CA

Your CA fingerprint has now changed.
