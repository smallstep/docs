...
`step-ca` can be exposed to the public internet.
This can be necessary for issuing network access certificates, in particular.

- Rather than exposing your entire CA to the internet,
  ...
  - `/sign` for JWK issuance
  - `/provisioners` enumerates all provisioners and allows remote access to encrypted JWKs — it is not necessary to expose this endpoint.
...
### How to Expose ACME provisioners

You may expose ACME provisioners (`/acme/*`), with precautions:
- The default ACME provisioner will issue certificates to any client, including clients from the public internet, just as Let's Encrypt does.
- Use [policies](./policies.mdx) to limit which subject names can request certificates via ACME.

#### Hiding your ACME provisioner

1. Obscure your ACME provisioner name, so that the directory URL is unguessable:
...
2. Do not expose the provisioner discovery endpoint. Run a reverse proxy in front of `step-ca`, and do not expose `/provisioners` to the internet. The ACME provisioner URL cannot be discovered.
