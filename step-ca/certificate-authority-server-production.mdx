...
This can be useful for issuing network access certificates, in particular.

Here are some guidelines for exposing a CA to the internet:
- Consider alternatives.
  - If you are using hardware-bound private keys in your clients,
    you could issue longer-lived certificates
    so that renewal from the public internet is never needed.
  - When your clients are spread across differenet networks—
...
- You may expose ACME provisioners (`/acme/*`), with precautions:
  ...
  - Do not expose `/provisioners`, so the ACME provisioner cannot be discovered
  - Obscure your ACME provisioner name, so that the directory URL is unguessable:

    ```
    {
    PROVISIONER_NAME=$(< /dev/urandom  LC_ALL=C tr -dc A-Za-z0-9 | head -c80)
    step ca provisioner update acme-da --name $PROVISIONER_NAME
