...
Here's an example of a Caddy reverse proxy configuration that uses ACME to get a leaf certificate from `step-ca`, for use with client connections into the proxy. In this setup, `step-ca` runs on the same machine, on localhost port 4443.
