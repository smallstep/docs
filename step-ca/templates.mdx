...
[Default X.509](https://github.com/smallstep/crypto/blob/master/x509util/templates.go)
...
- **options**: object that allows configuration of provisioner options, for example, templates.
  ...
  - **x509** or **ssh**: object for configuring X.509 or SSH certificate template options.
    ...
    - **templateFile**: path to a template stored on disk. You have a few options for how to define your path:
        ...
        - **_absolute path_**: for example, `/home/mariano/path/to/file.ptl`
        - **_relative to `$(step path)`_**: for example, `templates/certs/x509/leaf.tpl` the actual
          location of which would be `$(step path)/templates/certs/x509/leaf.tpl`.
        - **_relative to the execution directory of `step-ca`_**: for example, `./path/to/file.tpl` or `../path/to/file.tpl`
...
- `.Token`:
  If a signed token was used to obtain the certificate—for example, with the JWK provisioner—this property contains the payload of that token.
...
- `.Insecure.CR`<Reference id="star11" marker="*" />:
  ...
  Instead, `step-ca` creates a virtual certificate request,
...
least a `maxPathLen` of `1`. If you want to use an intermediate certificate
...
and then create intermediates that set a specific limit, that is, `0` for those that
