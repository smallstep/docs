...
Let's make a CSR, using [`step certificate create --csr`](../step-cli/reference/certificate/create.mdx), then we'll use [`step ca sign`](../step-cli/reference/ca/sign to get a signed certificate from the CA:
...
Under the hood, many of `step-ca`'s [certificate provisioners](../step-ca/certificate-authority-core-concepts/#provisioners) accept a Javascript Web Token (JWT) along with a CSR to authenticate certificate requests:
...
* Run [`step ca help`](../step-cli/reference/ca.mdx) or [`step ssh help`](../step-cli/reference/ssh for many more flags and examples.
