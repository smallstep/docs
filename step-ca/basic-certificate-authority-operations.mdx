...
Once you have a certificate authority up and running, the [`step ca certificate`](../step-cli/reference/ca/certificate.mdx) command is a one-step option for generating a private key and obtaining a signed certificate:
...
The positional arguments in the [`step ca certificate`](../step-cli/reference/ca/certificate.mdx) command indicate the name you would like associated with the certificate (in this case, the DNS host name of a service) and the output filenames for the certificate and private key.

You can check your work using [`step certificate inspect`](../step-cli/reference/certificate/inspect.mdx):
...
Let's make a CSR, using [`step certificate create --csr`](../step-cli/reference/certificate/create.mdx), then we'll use [`step ca sign`](../step-cli/reference/ca/sign.mdx) to get a signed certificate from the CA:
...
Note that because [`step ca renew`](../step-cli/reference/ca/renew.mdx) uses mutual TLS authentication, it can only renew client certificates (certificates that are marked with "Client Authentication" key usage). Any other certificate must be replaced before expiry instead.
...
When [`step ca init`](../step-cli/reference/ca/init.mdx) is run with `--ssh`, it creates two SSH CA key pairs: one for the host CA, and one for the user CA. The user CA key signs SSH user certificates, and the host CA key signs SSH host certificates.

### Requirements

* You will need a `step-ca` Certificate Authority **with SSH support enabled**. Create one by running [`step ca init --ssh`](../step-cli/reference/ca/init.mdx).
...
* Run [`step ca help`](../step-cli/reference/ca.mdx) or [`step ssh help`](../step-cli/reference/ssh.mdx) for many more flags and examples.
* Our [Tutorials](../tutorials.mdx) can guide you through more complex scenarios for using `step-ca` in various contexts and workflows.
