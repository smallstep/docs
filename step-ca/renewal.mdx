...
Now let's generate a [single-use bootstrap token](../cli/ca/token/.mdx) and use it to obtain a certificate:
...
or pass the `--not-after` flag to the [`step ca certificate`](../step-cli/reference/ca/certificate.mdx) to adjust the
...
Here are three options for setting up automated renewal of certificates using [`step ca renew`](../step-cli/reference/ca/renew.mdx):
...
With this method, [`step ca renew`](../step-cli/reference/ca/renew.mdx) operates as a daemon that will keep your certificates up-to-date:
...
When daemonized, [`step ca renew`](../step-cli/reference/ca/renew.mdx) will attempt a renewal when the certificate's lifetime is approximately two-thirds elapsed.
...
You can trigger renewal anytime by sending a `SIGHUP` signal to the [`step ca renew`](../step-cli/reference/ca/renew.mdx) process ID.

You can add [`step ca renew --daemon`](../step-cli/reference/ca/renew.mdx) as a `systemd` service that runs on startup and restarts as needed.
...
To address this, [`step ca renew`](../step-cli/reference/ca/renew.mdx) can send a `SIGHUP` to your service after each renewal.
...
With `cron`-based renewal, you can have [`step ca renew`](../step-cli/reference/ca/renew.mdx) run at a regular cadence (eg. every five minutes) and renew a certificate only when it's approaching its expiration date.
...
[`step ca renew`](../step-cli/reference/ca/renew.mdx) allows a certificate owner to extend the lifetime of a certificate before it expires.
...
To configure renewal after expiry for a provisioner, pass the `--allow-renewal-after-expiry` flag to the [`step ca provisioner`](../step-cli/reference/ca/provisioner.mdx) management commands. See [managing provisioners](../step-ca/provisioners/.mdx#managing-provisioners).
