...
Now let's generate a [single-use bootstrap token](../cli/ca/token/) and use it to obtain a certificate:
...
To configure renewal after expiry for a provisioner, pass the `--allow-renewal-after-expiry` flag to the [`step ca provisioner`](../step-cli/reference/ca/provisioner.mdx) management commands. See [managing provisioners](../step-ca/provisioners/#managing-provisioners).
