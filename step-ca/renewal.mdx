...
To configure renewal after expiry for a provisioner, pass the `--allow-renewal-after-expiry` flag to the [`step ca provisioner`]\(../step-cli/reference/ca/provisioner) management commands. See [managing provisioners](../step-ca/provisioners/#managing-provisioners).
