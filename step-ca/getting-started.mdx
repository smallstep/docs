...
Run the command [`step ca init`](/docs/step-cli/reference/ca/init) in a terminal to configure your CA. You'll be asked about your project, DNS setup, etc.

The [`step ca init`](/docs/step-cli/reference/ca/init) command has a lot of flags.
...
* Pass `--acme` to create an [ACME provisioner](/docs/step-ca/provisioners/#acme)
* Pass `--remote-management` to enable [Remote Provisioner Management](/docs/step-ca/provisioners/#remote-provisioner-management)
...
You can use [`step ca`](/docs/step-cli/reference/ca) and [`step ssh`](/docs/step-cli/reference/ssh) command groups to interact with the CA. See [Basic CA Operations](/docs/step-ca/basic-certificate-authority-operations) for an overview of common operations.
...
You can do this by supplying your CA fingerprint to [`step ca bootstrap`](/docs/step-cli/reference/ca/bootstrap).
...
If you already ran [`step certificate install`](/docs/step-cli/reference/certificate/install) to install your root CA into your system's trust store, this command will work and you'll see "Hello, World!".
