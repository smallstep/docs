...
So every use case has a step that is easy to skip:
...
So the presenter must send both its own certificate and the intermediate's certificate, and the verifier walks the chain:

```mermaid
flowchart LR
  leaf["Device certificate<br/>(sent by the device)"] -->|signed by| int["Intermediate<br/>(sent by the device)"]
  int -->|signed by| root["Root<br/>(in the verifier's trust store)"]
```
