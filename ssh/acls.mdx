...
### Step 2: Grant user group access to hosts
...
* **Optional**. Assign Certificate Principals. Smallstep now supports adding additional principals to user certificates. A **Principal** is a string that allows access to a host, or even a specific user on a host.
