...
Host tags are similar to EC2 Instance Tags. They’re used to define collections of hosts for [access control](./acls.mdx). You might assign tags identifying a host's role (for example, bastion, web, database), region (for example, us-east), environment (for example, staging, production), or any other relevant attribute. Let's look at an example:
