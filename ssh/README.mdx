...
<Alert severity="info">
  <AlertTitle>Smallstep SSH is now the SSH use case</AlertTitle>
  <div>Host management, single sign-on user certificates, and device-bound user certificates are one SSH use case on the platform. These pages stay while the guide that replaces them is written, and every link to them keeps working.</div>
</Alert>
