...
In this tutorial, we'll set up client certificate authentication with Smallstep, using an IPSec IKEv2 VPN.
...
2. Once the Gateway is deployed, gather the following properties for your Virtual Network Gateway:
   ...
   * The gateway's **Remote ID** and **Root CA Certificate**.
     These are located inside the Point-to-Site configuration settings file.
...
3. In Smallstep's Protect tab, create a [VPN resource](https://smallstep.com/app/?next=/protect/vpn):
   ...
   * Remote ID: Use the Remote ID you located earlier.
4. Choose "Next"
...
8. In Azure, follow the instructions in [Upload root certificate public key information](https://learn.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal#uploadfile) to add the Smallstep Root Certificate to the Root certificates section of your VPN's Point-to-site Configuration.
