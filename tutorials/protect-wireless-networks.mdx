...
Workspace ONE UEM can deploy the configuration profile Smallstep generates for your Wi-Fi resource,
which configures CA trust, certificate enrollment, and the Wi-Fi network on the device.

First, [connect Workspace ONE UEM to Smallstep](./connect-workspace-one-to-smallstep.mdx) if you haven't already.

Then open your Wi-Fi resource in the dashboard, under [Protect → Wi-Fi](https://smallstep.com/app/?next=/protect/wifi),
and download the configuration profile.

In Workspace ONE UEM:

1. Navigate to **Resources**
2. Select **Profiles** from the **Profiles & Baselines** section of the navigation
3. Select **Add**, then **Upload Profile**
4. Select **macOS**
5. Select **Device Profile**
6. Upload the configuration file you downloaded from the Smallstep Console, and click **Save**
7. Assign the uploaded profile to the desired device groups, and click **Publish**
