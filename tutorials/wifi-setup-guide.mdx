...
3. From the **Server Group** table, click the group you just created, then click on the **+** sign to add new RADIUS server details.
...
1. Go to **Security > RADIUS > Authentication, then c**lick **New** to add a new RADIUS server.
...
4. Go to the **Security** tab > **AAA Servers. I**n the **Server 1** dialog box, under **Authentication Servers,** select the RADIUS server that you just configured, and click **Apply.**
...
2. On the Default AAA Configuration page, scroll to **RADIUS Servers**, and click the **Add** button.
3. Provide the **RADIUS Server IP address, Authorization Client UDP Port** <*Smallstep’s RADIUS Port*>, and **Shared Secret** provided by **Smallstep.** 
4. Click **Save**. 
...
1. Navigate to **Networks** > **Add:**
    ...
    - For **Default Auth Role, s**elect **Enterprise User.**
...
Your new 802.1x EAP-TLS WPA-Enterprise Wi-Fi network is ready for use. See Juniper Mist reference documentation [here](https://www.juniper.net/documentation/us/en/software/mist/mist-wireless/topics/topic-map/radius-configuration.html).
...
3. On the **Add Authentication Server** dialogue box:
    ...
    3. **For Server,** click the “**+”** button to add a new RADIUS server IP address provided by Smallstep.
    4. **For Shared Secret, enter the s**hared RADIUS server secret provided by Smallstep.
4. Click **Save**.
...
3. On the **Add Wireless Network** dialog:
    1. For **Network name, e**nter a descriptive name for the network.
