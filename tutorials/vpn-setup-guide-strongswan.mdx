...
html_title: StrongSwan VPN with Certificates Guide
description: Configure StrongSwan VPN using Smallstep certificates. Secure remote access with certificate-based authentication for enterprises.
---
...
6. The `vpn.crt` file is a PEM bundle containing the server certificate and the Workloads Intermediate CA certificate. **strongSwan will only read the first certificate in `vpn.crt`. The Intermediate CA certificate will need to be in a separate file.**
