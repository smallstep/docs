...
Two ways exist to address this challenge.
Either:
...
to the top of your renewal configuration (for example, in `/etc/letsencrypt/renewal/foo.internal.conf`).
...
Renewals are slightly easier since `acme.sh` remembers to use the right root certificate. It can also remember how long you'd like to wait before renewing a certificate. Unfortunately, the [duration is specified in days](https://github.com/Neilpang/acme.sh/issues/2422) (via the `--days` flag) which is too coarse for `step-ca`'s default 24 hour certificate lifetimes. The easiest way to schedule renewals with `acme.sh` is to force them at a reasonable frequency, like every 8 hours, via cron:
...
Other good options for programmatic ACME in Go exist.
