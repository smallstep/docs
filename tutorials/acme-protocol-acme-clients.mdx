...
Replace `<step path>` with the output of the [`step path`](../step-cli/reference/path/.mdx) command.
...
You can use the `--exec` flag on the [`step ca renew`](../step-cli/reference/ca/renew.mdx) command to do this automatically:
