...
4. Create a plist file called `com.google.Chrome.plist`, and populate it with the following:
   ...
   <plist version="1.0">
     <dict>
       ...
       <array>
         <string>{"pattern":"[Server URL pattern]","filter":{"ISSUER":{"CN":"Smallstep [Team Slug] Accounts Intermediate CA"}}}</string>
       </array>
     </dict>
   </plist>
   ```
...
2. Navigate to the key for your browser.
...
by the [Smallstep extension for ChromeOS](https://chromewebstore.google.com/detail/smallstep-agent-for-chrom/khlfhcjblheppjmebghnnfgklpkofbob).
...
not through `AutoSelectCertificateForUrls`.
