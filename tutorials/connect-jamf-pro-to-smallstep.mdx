...
- **Option 1** (below): Use Jamf Pro's package distribution and policy management
- **Option 2**: Use a separate software management tool like [Munki](https://www.munki.org/munki/), or install manually via scripts

For Option 2, see the [Smallstep Agent Manual Installation](../platform/smallstep-agent.mdx#macos-installation) guide for detailed macOS installation instructions.
