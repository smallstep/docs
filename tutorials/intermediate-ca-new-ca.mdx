...
It's easiest to run [`step ca init`](../step-cli/reference/ca/init.mdx) to get the boilerplate configuration in place, then remove or replace these artifacts with new ones that are tied to your existing root CA.

```shell-session 
$ step ca init
```

When you run [`step ca init`](../step-cli/reference/ca/init.mdx) we create a couple artifacts under `~/.step/`. The important ones for us are:
...
Now you need to generate a new signing key and intermediate certificate signed by your existing root CA. To do that, we can use the [`step certificate create`](../step-cli/reference/certificate/create.mdx) subcommand to generate a certificate signing request (CSR) that we'll have your existing root CA sign, producing an intermediate certificate.
