...
If `step-ca` raises an OIDC token validation error, you can examine the token you receive from Keycloak using the [`step oauth`](../step-cli/reference/oauth/.mdx) command. For example:
...
* The email address used with [`step ssh login`](../step-cli/reference/ssh/login.mdx) doesn't match the email address of the Keycloak user.
