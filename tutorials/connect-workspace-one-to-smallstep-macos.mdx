...
title: Connect Workspace One UEM to Smallstep (macOS)
html_title: Omnissa Workspace ONE macOS Integration Guide
description: Connect Workspace ONE UEM to Smallstep for unified device identity on macOS. Enterprise guide for Apple device security management.
---
...
This guide covers deploying the Smallstep Agent to **macOS** endpoints with Workspace ONE UEM. If you are managing Windows devices, see [Connect Workspace One UEM to Smallstep](./connect-workspace-one-to-smallstep.mdx) instead.
...
**Note:** if you don't want to use Workspace ONE UEM to install the agent, see the [Smallstep Agent Manual Installation](../platform/smallstep-agent.mdx#macos-installation) guide.

- [Download the Smallstep Agent for macOS (PKG)](https://packages.smallstep.com/stable/darwin/step-agent_latest.pkg)
...
2. Choose the groups you’d like to assign the application to. Assign the app to a single device or a small group of test devices for a staged rollout.
...
2. Choose **+ Add**
    1. Provide a name for the CA connection, for example, Smallstep Agents CA
...
1. Provide a name for the Template, for example, `Smallstep Agents`
...
1. In Workspace One UEM,
    ...
    3. Click **Apple macOS**, and then select **Device Profile**
    4. Under General, Provide a name (for example, "Smallstep Device Enrollment")
        1. Select the appropriate group in the Smart Groups select list
    ...
    5. Select the **SCEP** payload type on the left and choose **Configure**. Set the following settings:
    ...
    6. Select the **Custom Settings** payload type on the left and choose **Configure**. This delivers the Smallstep Agent configuration as a managed preference in the `com.smallstep.Agent` domain.
        ...
        - The `Certificate` value is a KMS URI that points the agent at the identity installed by the SCEP payload above. The `label` must match the keychain label Workspace ONE assigns to that identity; adjust it to the lookup value your Workspace ONE tenant uses if `$PROFILE_IDENTIFIER` does not resolve.
