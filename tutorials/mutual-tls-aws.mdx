...
Terraform uses a backend
hosted by [HashiCorp](https://app.terraform.io/session)
to store managed infrastructure state
and manage concurrency locks
that allow only one team member
to perform changes at a time.
The CLI needs a user configuration
as outlined below.
Create a user account and organization
at [app.terraform.io](https://app.terraform.io/session)
and get a user token.
For more details,
see [HashiCorp's Terraform CLI docs](https://www.terraform.io/docs/commands/cli-config.html).
