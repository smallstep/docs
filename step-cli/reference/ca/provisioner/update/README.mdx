...
**--remove-scope**=`scope`
Remove the `scope` used to validate the scopes extension in an OpenID Connect token.
...
Exclude the CA intermediate certificate in the SCEP CA certificate chain
...
The path to a PEM certificate `file` for the SCEP decrypter

**--scep-decrypter-key-file**=`file`
The path to a PEM private key `file` for the SCEP decrypter

**--scep-decrypter-key-uri**=`uri`
The key `uri` for the SCEP decrypter. Should be a valid value for the KMS type used.

**--scep-decrypter-key-password-file**=`file`
The path to a `file` containing the password for the SCEP decrypter key
...
**--gcp-organization**=`id`
The Google organization `id` used to validate the project in the identity tokens.
...
Disable the Smallstep extension for all certificates generated by this provisioner.
