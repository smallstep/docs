...
html_title: The Step Command Complete Overview Guide
description: Master the step command for certificate operations. Core concepts and usage patterns for effective CLI certificate management.
---
...
If you regularly interact with multiple CAs, we recommend enabling _contexts_.

Contexts let you configure and select multiple CAs and configuration profiles.
When this optional feature is enabled, the `$HOME/.step` folder will have a different layout.
...
Contexts are created when `--context [name]` is passed to any of the following:
...
When contexts are enabled, there's always a currently active context ([`step context current`](./reference/context/current)), and you can pass a `--context` name to any [`step ca`](./reference/ca), [`step ssh`](./reference/ssh/), or `step-ca` command to select a different context for a single operation.

Use the [`step context`](./reference/context/) command group to switch contexts, remove a context and its associated configuration, and view the current context. The [`step path`](./reference/path/) command shows the current context's configuration path. To display the top-level configuration directory name (`$STEPPATH`) when contexts are enabled, use `step path --base`.
...
- Client configurations are stored in a `profiles` directory, and CA server configurations and data are stored in an `authorities` directory.
- Two [`defaults.json`](#configuration-file) files are created: One in the `profiles` tree, and one in the `authorities` tree. They are merged at runtime, and the defaults in `profiles` take precedence.

### Migrating to Contexts

No support exists for automatically migrating a single-profile `$STEPPATH` configuration into its own context.
