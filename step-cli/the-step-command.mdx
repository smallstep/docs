...
- There is always a currently active context ([`step context current`](../step-cli/reference/context/current.mdx)), but you can pass a `--context` name to any [`step ca`](../step-cli/reference/ca.mdx), [`step ssh`](../step-cli/reference/ssh/), or `step-ca` command to temporarily select a context for a single operation.
